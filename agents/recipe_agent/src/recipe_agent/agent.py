...
class RecipeExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue):
        ...
        await task_updater.update_status(
            ...
            new_text_message(
                "Thinking about recipes...", context_id=task_updater.context_id, task_id=task_updater.task_id
            ),
        )
...
def run():
    ...
    request_handler = DefaultRequestHandler(
        agent_executor=RecipeExecutor(), task_store=InMemoryTaskStore(), agent_card=agent_card
    )
